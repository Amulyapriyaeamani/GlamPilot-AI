...
*   **AI Consultation & Planner:** Diagnostic form that matches target budget limits with aesthetic goals (e.g., Bridal, Corporate, Casual, Festive).
...
3.  **Deep Dive:** Customer inspects the selected salon's strengths and reads the AI-generated review summaries.
...
*   **Styling:** Tailwind CSS (Fluid design, glassmorphic panel styles, and fine-grain micro-interactions)
...
## Future Roadmap
1.  **Virtual Try-On AR Lens:** An interactive try-on simulator using media-pipe facial mesh nodes for exact real-time camera overlays (hair color highlights, blush, hairstyle contours).
2.  **Google Maps Platform Integration:** Real-time distance matrix routing to guide clients to salons based on current Hyderabad traffic.
